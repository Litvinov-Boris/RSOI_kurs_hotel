...
def login(request):  #
    ...
    """
    session = requests.post("http://litvinov-session.herokuapp.com/api/v1/session/login",
...
def register(request):  #
    ...
    """
    session = requests.post("http://litvinov-session.herokuapp.com/api/v1/session/register", json=request.data)
    if session.status_code != 200:
        return JsonResponse(session.json(), status=status.HTTP_400_BAD_REQUEST)
    session = session.json()["user_uid"]
    request.data.update({"user_uid": session})
    loyalty = requests.post("http://litvinov-loyalty.herokuapp.com/api/v1/loyalty/create", json=request.data)
...
def logout(request):  #
    ...
    """
    session = requests.post("http://litvinov-session.herokuapp.com/api/v1/session/logout", cookies=request.COOKIES)
    if session.status_code != 200:
        return JsonResponse(session.json(), status=status.HTTP_400_BAD_REQUEST)
    response = JsonResponse({'success': 'logout'}, status=status.HTTP_200_OK)

    user = requests.get("http://litvinov-session.herokuapp.com/api/v1/session/user/{}".format(session.json()["user_uid"]),
...
def users(request):
    ...
    GET: use JWT
    """
    session = requests.get("http://litvinov-session.herokuapp.com/api/v1/session/validate", cookies=request.COOKIES)
    if session.status_code != 200:
        if session.status_code == 403:
            session = requests.get("http://litvinov-session.herokuapp.com/api/v1/session/refresh", cookies=request.COOKIES)
        else:
            return JsonResponse({"error": "Internal error"}, status=status.HTTP_400_BAD_REQUEST)
    _users = requests.get("http://litvinov-session.herokuapp.com/api/v1/session/users", cookies=session.cookies)
...
def add_hotel(request):
    ...
    if session.status_code != 200:
        if session.status_code == 403:
            ...
            return JsonResponse({"error": "Internal error"}, status=status.HTTP_400_BAD_REQUEST)
    hotel = requests.post("http://litvinov-hotel.herokuapp.com/api/v1/hotels/", json=request.data, cookies=session.cookies)
...
def all_hotels(request):
    ...
    if session.status_code != 200:
        if session.status_code == 403:
            ...
            return JsonResponse({"error": "Internal error"}, status=status.HTTP_400_BAD_REQUEST)
    hotel = requests.get("http://litvinov-hotel.herokuapp.com/api/v1/hotels", json=request.data, cookies=session.cookies)
    if hotel.status_code != 200:
        return JsonResponse(hotel.json(), status=status.HTTP_400_BAD_REQUEST)
    response = JsonResponse(hotel.json(), status=status.HTTP_200_OK, safe=False)
    response.set_cookie(key='jwt', value=session.cookies.get('jwt'), httponly=True)
...
def one_hotel_or_delete(request, hotel_uid):
    ...
    if request.method == 'GET':
        hotel = requests.get("http://litvinov-hotel.herokuapp.com/api/v1/hotels/{}"
                             .format(hotel_uid), json=request.data, cookies=session.cookies)
        response = JsonResponse(hotel.json(), status=status.HTTP_200_OK, safe=False)
    else:  # DELETE
        hotel = requests.delete("http://litvinov-hotel.herokuapp.com/api/v1/hotels/{}"
                                .format(hotel_uid), json=request.data, cookies=session.cookies)
...
def create_booking_or_all(request):
    ...
    if session.status_code != 200:
        if session.status_code == 403:
            ...
            return JsonResponse({"error": "Internal error"}, status=status.HTTP_400_BAD_REQUEST)
    if request.method == 'GET':
        booking = requests.get("http://litvinov-booking.herokuapp.com/api/v1/booking/", cookies=session.cookies)
        ...
        #  создаем бронь
        booking = requests.post("http://litvinov-booking.herokuapp.com/api/v1/booking/", json=request.data, cookies=session.cookies)
        if booking.status_code != 200:
            return JsonResponse(booking.json(), status=status.HTTP_400_BAD_REQUEST)
        #  подсчитываем количество броней для определения нужно ли повышать лояльность или нет
        booking_all = requests.get("http://litvinov-booking.herokuapp.com/api/v1/booking/", cookies=session.cookies)
        ...
        l_status = requests.get("http://litvinov-loyalty.herokuapp.com/api/v1/loyalty/balance", cookies=session.cookies)
        ...
        if 1 < len(len_booking) < 35 and l_status == 'None':  # BRONZE
            loyaltyUP = requests.patch("http://litvinov-loyalty.herokuapp.com/api/v1/loyalty/edit", json={"active": "UP"},
        ...
        payBalance = requests.get("http://litvinov-payment.herokuapp.com/api/v1/payment/status/{}".format(booking.get("payment_uid")),
                                  cookies=request.COOKIES)
        if payBalance.status_code == 200:
            payBalance = payBalance.json()
            booking.update(payBalance)
        about_hotel = requests.get("http://litvinov-hotel.herokuapp.com/api/v1/hotels/{}".format(booking.get("hotel_uid")),
                                   cookies=request.COOKIES)
        if about_hotel.status_code == 200:
            about_hotel = about_hotel.json()
            booking.update(about_hotel)
        user = requests.get("http://litvinov-session.herokuapp.com/api/v1/session/user/{}".format(booking.get("user_uid")),
                            cookies=request.COOKIES)
        if user.status_code == 200:
            user = user.json()
            booking.update(user)
        loyalty = requests.get("http://litvinov-loyalty.herokuapp.com/api/v1/loyalty/status/{}".format(booking.get("user_uid")),
                               cookies=request.COOKIES)
        if loyalty.status_code == 200:
            loyalty = loyalty.json()
            booking.update(loyalty)

    producer(booking, '4dvdu2cf-payment')
...
def one_booking(request, booking_uid):
    ...
    if session.status_code != 200:
        if session.status_code == 403:
            ...
            return JsonResponse({"error": "Internal error"}, status=status.HTTP_400_BAD_REQUEST)
    booking_one = requests.get("http://litvinov-booking.herokuapp.com/api/v1/booking/{}".format(booking_uid), cookies=session.cookies)
...
def all_booking_hotels(request, hotel_uid):
    ...
    if session.status_code != 200:
        if session.status_code == 403:
            ...
            return JsonResponse({"error": "Internal error"}, status=status.HTTP_400_BAD_REQUEST)
    booking_hotel = requests.get("http://litvinov-booking.herokuapp.com/api/v1/booking/hotels/{}".format(hotel_uid),
                                 cookies=session.cookies)
...
def pay_booking(request, booking_uid):
    ...
    if session.status_code != 200:
        if session.status_code == 403:
            ...
            return JsonResponse({"error": "Internal error"}, status=status.HTTP_400_BAD_REQUEST)
    booking_status = requests.get("http://litvinov-booking.herokuapp.com/api/v1/booking/{}".format(booking_uid),
                                  cookies=session.cookies)
    ...
    booking_pay = requests.post("http://litvinov-booking.herokuapp.com/api/v1/booking/pay/{}".format(booking_uid),
                                cookies=session.cookies)
...
def close_booking(request, booking_uid):
    ...
    if booking_status == 'PAID' and booking_status != 'REVERSED' and booking_status != 'CANCELED':
        booking_r = requests.post("http://litvinov-booking.herokuapp.com/api/v1/booking/reversed/{}".format(booking_uid),
                                  cookies=session.cookies)
    ...
    if booking_status == 'NEW' and booking_status != 'REVERSED' and booking_status != 'CANCELED':
        booking_r = requests.post("http://litvinov-booking.herokuapp.com/api/v1/booking/canceled/{}".format(booking_uid),
                                  cookies=session.cookies)
        if booking_r.status_code != 200:
            return JsonResponse(booking_r.json(), status=status.HTTP_400_BAD_REQUEST)
...
def report_booking(request):
    ...
    report = requests.get("http://https://litvinov-report.herokuapp.com//api/v1/reports/booking", cookies=session.cookies)
...
def report_user(request):
    ...
    report = requests.get("http://https://litvinov-report.herokuapp.com//api/v1/reports/users", cookies=session.cookies)
    if report.status_code == 200:
        report = report.content.decode('utf8').replace("'", '"')
        report = json.loads(report)
        return JsonResponse(report, status=status.HTTP_200_OK)
...
def report_hotels(request):
    ...
    if session.status_code != 200:
        if session.status_code == 403:
            ...
            return JsonResponse({"error": "Internal error"}, status=status.HTTP_400_BAD_REQUEST)
    # достаем отчет по пользователям: логирование, разлогирование, регистрация
    report = requests.get("http://https://litvinov-report.herokuapp.com//api/v1/reports/hotels", cookies=session.cookies)
    if report.status_code == 200:
        report = report.content.decode('utf8').replace("'", '"')
        report = json.loads(report)
...
def cities(request):
    dict_cities = requests.get("http://litvinov-hotel.herokuapp.com/api/v1/hotels/cities")
...
def add_hotlike(request):
    auth(request)
    hotellikes = requests.post("http://litvinov-rating.herokuapp.com/api/v1/rating/add_hotlike",
...
def add_comlike(request):
    auth(request)
    commentlikes = requests.patch("http://litvinov-rating.herokuapp.com/api/v1/rating/add_comlike",
...
def show_hotlikes(request):
    hotellikes = requests.get("http://litvinov-rating.herokuapp.com/api/v1/rating/show_hotlikes",
                               json={'hotel_uid': request.POST['hotel_uid']},
                               cookies=request.COOKIES).json()
...
def show_comlikes(request):
    commentlikes = requests.get("http://litvinov-rating.herokuapp.com/api/v1/rating/show_comlikes",
                               json={'comment_uid': request.POST['comment_uid']},
                               cookies=request.COOKIES).json()
...
def delete_comment(request):
    del_comment = requests.delete("http://litvinov-rating.herokuapp.com/api/v1/rating/delete_comment",
                                  json={'comment_uid': request.POST['comment_uid']},
                                  cookies=request.COOKIES).json()
...
def index(request):
    ...
    data = auth(request)
    cities = requests.get("http://litvinov-hotel.herokuapp.com/api/v1/hotels/cities").json()
    _allhotels = requests.get("http://litvinov-hotel.herokuapp.com/api/v1/hotels", cookies=request.COOKIES).json()
...
def make_login(request):
    ...
    if request.method == "POST":
        ...
        session = requests.post('http://litvinov-gateway.herokuapp.com/api/v1/login',
...
def hotel_info(request, hotel_uid):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                new_comment = requests.post("http://litvinov-rating.herokuapp.com/api/v1/rating/create_comment",
        ...
        hotellikes = requests.get("http://litvinov-rating.herokuapp.com/api/v1/rating/load_hotlikes",
                                  json={'hotel_uid': hotel_uid},
                                  cookies=request.COOKIES).json()
        _hotel_comments = requests.get("http://litvinov-rating.herokuapp.com/api/v1/rating/load_comments",
                                  json={'hotel_uid': hotel_uid},
                                  cookies=request.COOKIES).json()
...
def add_booking(request):
    ...
    if request.method == 'POST':
        ...
        if datetime.datetime.strptime(request.POST['date_start'], "%Y-%m-%d") > datetime.datetime.strptime(
            ...
            booking = requests.post("http://litvinov-booking.herokuapp.com/api/v1/booking/",
...
def booking_info(request, booking_uid):
    ...
    cities = requests.get("http://litvinov-hotel.herokuapp.com/api/v1/hotels/cities").json()
    try:
        booking = requests.get("http://litvinov-booking.herokuapp.com/api/v1/booking/{}"
                               .format(booking_uid), cookies=session.cookies).json()
        hotel = requests.get("http://litvinov-hotel.herokuapp.com/api/v1/hotels/{}"
                             .format(booking['hotel_uid']), cookies=session.cookies).json()
        payment = requests.get("http://litvinov-payment.herokuapp.com/api/v1/payment/status/{}"
...
def pay_room(request, payment_uid):
    ...
    if request.method == 'POST':
        ...
        period = date_end - date_start
        totalcost = int(hotel['cost']) * (period.days)
        pay = requests.post("http://litvinov-payment.herokuapp.com/api/v1/payment/pay/{}"
...
def del_booking(request, booking_uid):
    ...
    if request.method == "POST":
        ...
        if request.POST['status'] == "NEW":
            delbook = requests.delete("http://litvinov-booking.herokuapp.com/api/v1/booking/canceled/{}"
            ...
            period = date_end - date_start
            totalcost = int(hotel['cost']) * (period.days)
            payment = requests.post("http://litvinov-payment.herokuapp.com/api/v1/payment/reversed/{}"
...
def search_hotel_booking(request):
    ...
    if request.method == 'POST':
        ...
        if datetime.datetime.strptime(request.POST['date_start'], "%Y-%m-%d") > datetime.datetime.strptime(
            ...
            search = requests.post("http://litvinov-hotel.herokuapp.com/api/v1/hotels/date",
    ...
    if request.method == "POST":
        ...
        try:
            ...
            with open(f'gateway/static/images/{filename}', 'wb') as image:
                ...
                image.write(files)
            new_hotel = requests.post("http://litvinov-hotel.herokuapp.com/api/v1/hotels/",
...
def delete_hotel_admin(request):
    ...
    if request.method == "POST":
        ...
        new_hotel = requests.delete('http://litvinov-gateway.herokuapp.com/api/v1/hotels/{}'.format(form.data['hotel_uid']),
                                    cookies=request.COOKIES)
        error = 'success'
...
def all_users(request):
    ...
    if data['role'] != 'admin':
        ...
        response.set_cookie(key='jwt', value=session.cookies.get('jwt'), httponly=True)
        return response
    _users = requests.get("http://litvinov-gateway.herokuapp.com/api/v1/users", cookies=request.COOKIES).json()
...
def users_static(request):
    ...
    if data['role'] != 'admin':
        ...
        return response
    try:
        static_users = requests.get("http://litvinov-gateway.herokuapp.com/api/v1/reports/users", cookies=request.COOKIES).json()
        dictlist = list()
...
def all_booking_static(request):
    ...
    if data['role'] != 'admin':
        ...
        return response
    try:
        static_booking = requests.get("http://https://litvinov-report.herokuapp.com//api/v1/reports/hotels", cookies=request.COOKIES).json()
    ...
    if request.method == "POST":
        if len(request.POST['hotel_uid']) > 0:
            if request.POST['status'] == "all":
                try:
                    s = requests.get("http://https://litvinov-report.herokuapp.com//api/v1/reports/hotels", cookies=request.COOKIES).json()
                    static_booking = []
                    for static in s:
            ...
            if request.POST['status'] == "all":
                try:
                    static_booking = requests.get("http://https://litvinov-report.herokuapp.com//api/v1/reports/hotels",
                                                  cookies=request.COOKIES).json()
                    static_booking = sorted(static_booking, key=lambda k: k['hotel_uid'])
                except Exception:
                    static_booking = None
...
def make_logout(request):
    session = requests.get("http://litvinov-gateway.herokuapp.com/api/v1/logout", cookies=request.COOKIES)
    if session.status_code == 200:
        response = HttpResponseRedirect('/index')
        response.delete_cookie('jwt')
        return response
...
def balance(request):
    ...
    data = auth(request)
    try:
        loyalty = requests.get("http://litvinov-loyalty.herokuapp.com/api/v1/loyalty/status/{}".format(data['user_uid']),
                               cookies=request.COOKIES).json()
        user = requests.get("http://litvinov-session.herokuapp.com/api/v1/session/user/{}".format(data['user_uid']),
                            cookies=request.COOKIES).json()
        _allbook = requests.get("http://litvinov-booking.herokuapp.com/api/v1/booking/", cookies=request.COOKIES).json()
        ...
        for s in sort:
            ...
            if datetime.datetime.strptime(s['date_end'], "%Y-%m-%d") > datetime.datetime.now() \
                ...
                ch = requests.get("http://litvinov-hotel.herokuapp.com/api/v1/hotels/{}"
                ...
                hh = requests.get("http://litvinov-hotel.herokuapp.com/api/v1/hotels/{}"
                                  .format(s['hotel_uid']), cookies=session.cookies).json()
                hist.append(s)
                histhotel.append(hh)
...
def registration(request):
    ...
    if request.method == "POST":
        ...
        with open(f'gateway/static/images/avatars/{filename}', 'wb') as image:
            ...
            image.write(files)
        session = requests.post('http://litvinov-gateway.herokuapp.com/api/v1/register',
