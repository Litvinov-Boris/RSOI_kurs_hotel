...
def register(request):  #
    ...
    loyalty = requests.post("https://litvinov-loyalty.herokuapp.com/api/v1/loyalty/create", json=request.data)
...
def logout(request):  #
    ...
    user = requests.get("https://litvinov-session.herokuapp.com/api/v1/session/user/{}".format(session.json()["user_uid"]),
...
def users(request):
    ...
    GET: use JWT
    """
    session = requests.get("https://litvinov-session.herokuapp.com/api/v1/session/validate", cookies=request.COOKIES)
    if session.status_code != 200:
        if session.status_code == 403:
            session = requests.get("https://litvinov-session.herokuapp.com/api/v1/session/refresh", cookies=request.COOKIES)
        else:
            return JsonResponse({"error": "Internal error"}, status=status.HTTP_400_BAD_REQUEST)
    _users = requests.get("https://litvinov-session.herokuapp.com/api/v1/session/users", cookies=session.cookies)
...
def add_hotel(request):
    ...
    if session.status_code != 200:
        if session.status_code == 403:
            ...
            return JsonResponse({"error": "Internal error"}, status=status.HTTP_400_BAD_REQUEST)
    hotel = requests.post("https://litvinov-hotel.herokuapp.com/api/v1/hotels/", json=request.data, cookies=session.cookies)
...
def all_hotels(request):
    ...
    if session.status_code != 200:
        if session.status_code == 403:
            ...
            return JsonResponse({"error": "Internal error"}, status=status.HTTP_400_BAD_REQUEST)
    hotel = requests.get("https://litvinov-hotel.herokuapp.com/api/v1/hotels", json=request.data, cookies=session.cookies)
    if hotel.status_code != 200:
        return JsonResponse(hotel.json(), status=status.HTTP_400_BAD_REQUEST)
    response = JsonResponse(hotel.json(), status=status.HTTP_200_OK, safe=False)
    response.set_cookie(key='jwt', value=session.cookies.get('jwt'), httponly=True)
...
def one_hotel_or_delete(request, hotel_uid):
    ...
    if request.method == 'GET':
        hotel = requests.get("https://litvinov-hotel.herokuapp.com/api/v1/hotels/{}"
                             .format(hotel_uid), json=request.data, cookies=session.cookies)
        response = JsonResponse(hotel.json(), status=status.HTTP_200_OK, safe=False)
    else:  # DELETE
        hotel = requests.delete("https://litvinov-hotel.herokuapp.com/api/v1/hotels/{}"
                                .format(hotel_uid), json=request.data, cookies=session.cookies)
...
def create_booking_or_all(request):
    ...
    if session.status_code != 200:
        if session.status_code == 403:
            ...
            return JsonResponse({"error": "Internal error"}, status=status.HTTP_400_BAD_REQUEST)
    if request.method == 'GET':
        booking = requests.get("https://litvinov-booking.herokuapp.com/api/v1/booking/", cookies=session.cookies)
        ...
        #  создаем бронь
        booking = requests.post("https://litvinov-booking.herokuapp.com/api/v1/booking/", json=request.data, cookies=session.cookies)
        if booking.status_code != 200:
            return JsonResponse(booking.json(), status=status.HTTP_400_BAD_REQUEST)
        #  подсчитываем количество броней для определения нужно ли повышать лояльность или нет
        booking_all = requests.get("https://litvinov-booking.herokuapp.com/api/v1/booking/", cookies=session.cookies)
        ...
        l_status = requests.get("https://litvinov-loyalty.herokuapp.com/api/v1/loyalty/balance", cookies=session.cookies)
        ...
        if 20 < len(len_booking) < 35 and l_status == 'None':  # BRONZE
            loyaltyUP = requests.patch("https://litvinov-loyalty.herokuapp.com/api/v1/loyalty/edit", json={"active": "UP"},
        ...
        payBalance = requests.get("https://litvinov-payment.herokuapp.com/api/v1/payment/status/{}".format(booking.get("payment_uid")),
                                  cookies=request.COOKIES)
        if payBalance.status_code == 200:
            payBalance = payBalance.json()
            booking.update(payBalance)
        about_hotel = requests.get("https://litvinov-hotel.herokuapp.com/api/v1/hotels/{}".format(booking.get("hotel_uid")),
                                   cookies=request.COOKIES)
        if about_hotel.status_code == 200:
            about_hotel = about_hotel.json()
            booking.update(about_hotel)
        user = requests.get("https://litvinov-session.herokuapp.com/api/v1/session/user/{}".format(booking.get("user_uid")),
                            cookies=request.COOKIES)
        if user.status_code == 200:
            user = user.json()
            booking.update(user)
        loyalty = requests.get("https://litvinov-loyalty.herokuapp.com/api/v1/loyalty/status/{}".format(booking.get("user_uid")),
                               cookies=request.COOKIES)
        if loyalty.status_code == 200:
...
def one_booking(request, booking_uid):
    ...
    if session.status_code != 200:
        if session.status_code == 403:
            ...
            return JsonResponse({"error": "Internal error"}, status=status.HTTP_400_BAD_REQUEST)
    booking_one = requests.get("https://litvinov-booking.herokuapp.com/api/v1/booking/{}".format(booking_uid), cookies=session.cookies)
...
def all_booking_hotels(request, hotel_uid):
    ...
    if session.status_code != 200:
        if session.status_code == 403:
            ...
            return JsonResponse({"error": "Internal error"}, status=status.HTTP_400_BAD_REQUEST)
    booking_hotel = requests.get("https://litvinov-booking.herokuapp.com/api/v1/booking/hotels/{}".format(hotel_uid),
                                 cookies=session.cookies)
...
def pay_booking(request, booking_uid):
    ...
    if session.status_code != 200:
        if session.status_code == 403:
            ...
            return JsonResponse({"error": "Internal error"}, status=status.HTTP_400_BAD_REQUEST)
    booking_status = requests.get("https://litvinov-booking.herokuapp.com/api/v1/booking/{}".format(booking_uid),
                                  cookies=session.cookies)
    ...
    booking_pay = requests.post("https://litvinov-booking.herokuapp.com/api/v1/booking/pay/{}".format(booking_uid),
                                cookies=session.cookies)
...
def close_booking(request, booking_uid):
    ...
    if booking_status == 'PAID' and booking_status != 'REVERSED' and booking_status != 'CANCELED':
        booking_r = requests.post("https://litvinov-booking.herokuapp.com/api/v1/booking/reversed/{}".format(booking_uid),
                                  cookies=session.cookies)
    ...
    if booking_status == 'NEW' and booking_status != 'REVERSED' and booking_status != 'CANCELED':
        booking_r = requests.post("https://litvinov-booking.herokuapp.com/api/v1/booking/canceled/{}".format(booking_uid),
                                  cookies=session.cookies)
        if booking_r.status_code != 200:
            return JsonResponse(booking_r.json(), status=status.HTTP_400_BAD_REQUEST)
...
def report_booking(request):
    ...
    report = requests.get("https://litvinov-report.herokuapp.com/api/v1/reports/booking", cookies=session.cookies)
...
def report_user(request):
    ...
    report = requests.get("https://litvinov-report.herokuapp.com/api/v1/reports/users", cookies=session.cookies)
    if report.status_code == 200:
        report = report.content.decode('utf8').replace("'", '"')
        report = json.loads(report)
        return JsonResponse(report, status=status.HTTP_200_OK)
...
def report_hotels(request):
    ...
    if session.status_code != 200:
        if session.status_code == 403:
            ...
            return JsonResponse({"error": "Internal error"}, status=status.HTTP_400_BAD_REQUEST)
    # достаем отчет по пользователям: логирование, разлогирование, регистрация
    report = requests.get("https://litvinov-report.herokuapp.com/api/v1/reports/hotels", cookies=session.cookies)
    if report.status_code == 200:
        report = report.content.decode('utf8').replace("'", '"')
        report = json.loads(report)
...
def cities(request):
    dict_cities = requests.get("https://litvinov-hotel.herokuapp.com/api/v1/hotels/cities")
...
def index(request):
    ...
    data = auth(request)
    cities = requests.get("https://litvinov-hotel.herokuapp.com/api/v1/hotels/cities").json()
...
def add_booking(request):
    ...
    if request.method == 'POST':
        ...
        if datetime.datetime.strptime(request.POST['date_start'], "%Y-%m-%d") > datetime.datetime.strptime(
            ...
            booking = requests.post("https://litvinov-booking.herokuapp.com/api/v1/booking/",
...
def booking_info(request, booking_uid):
    ...
    cities = requests.get("https://litvinov-hotel.herokuapp.com/api/v1/hotels/cities").json()
    try:
        booking = requests.get("https://litvinov-booking.herokuapp.com/api/v1/booking/{}"
                               .format(booking_uid), cookies=session.cookies).json()
        hotel = requests.get("https://litvinov-hotel.herokuapp.com/api/v1/hotels/{}"
                             .format(booking['hotel_uid']), cookies=session.cookies).json()
        payment = requests.get("https://litvinov-payment.herokuapp.com/api/v1/payment/status/{}"
...
def pay_room(request, payment_uid):
    ...
    if request.method == 'POST':
        ...
        period = date_end - date_start
        totalcost = int(hotel['cost']) * (period.days)
        pay = requests.post("https://litvinov-payment.herokuapp.com/api/v1/payment/pay/{}"
...
def del_booking(request, booking_uid):
    ...
    if request.method == "POST":
        ...
        if request.POST['status'] == "NEW":
            delbook = requests.delete("https://litvinov-booking.herokuapp.com/api/v1/booking/canceled/{}"
            ...
            period = date_end - date_start
            totalcost = int(hotel['cost']) * (period.days)
            payment = requests.post("https://litvinov-payment.herokuapp.com/api/v1/payment/reversed/{}"
...
def search_hotel_booking(request):
    ...
    if request.method == 'POST':
        ...
        if datetime.datetime.strptime(request.POST['date_start'], "%Y-%m-%d") > datetime.datetime.strptime(
            ...
            search = requests.post("https://litvinov-hotel.herokuapp.com/api/v1/hotels/date",
    ...
    if request.method == "POST":
        ...
        try:
            ...
            with open(f'gateway/static/images/{filename}', 'wb') as image:
                ...
                image.write(files)
            new_hotel = requests.post("https://litvinov-hotel.herokuapp.com/api/v1/hotels/",
...
def delete_hotel_admin(request):
    ...
    if request.method == "POST":
        ...
        new_hotel = requests.delete('https://litvinov-gateway.herokuapp.com/api/v1/hotels/{}'.format(form.data['hotel_uid']),
                                    cookies=request.COOKIES)
        error = 'success'
...
def users_static(request):
    ...
    if data['role'] != 'admin':
        ...
        return response
    try:
        static_users = requests.get("https://litvinov-gateway.herokuapp.com/api/v1/reports/users", cookies=request.COOKIES).json()
        dictlist = list()
...
def all_booking_static(request):
    ...
    if data['role'] != 'admin':
        ...
        return response
    try:
        static_booking = requests.get("https://litvinov-report.herokuapp.com/api/v1/reports/hotels", cookies=request.COOKIES).json()
    ...
    if request.method == "POST":
        if len(request.POST['hotel_uid']) > 0:
            if request.POST['status'] == "all":
                try:
                    s = requests.get("https://litvinov-report.herokuapp.com/api/v1/reports/hotels", cookies=request.COOKIES).json()
                    static_booking = []
                    for static in s:
            ...
            if request.POST['status'] == "all":
                try:
                    static_booking = requests.get("https://litvinov-report.herokuapp.com/api/v1/reports/hotels",
                                                  cookies=request.COOKIES).json()
                    static_booking = sorted(static_booking, key=lambda k: k['hotel_uid'])
                except Exception:
                    static_booking = None
...
def balance(request):
    ...
    data = auth(request)
    try:
        loyalty = requests.get("https://litvinov-loyalty.herokuapp.com/api/v1/loyalty/status/{}".format(data['user_uid']),
                               cookies=request.COOKIES).json()
        user = requests.get("https://litvinov-session.herokuapp.com/api/v1/session/user/{}".format(data['user_uid']),
                            cookies=request.COOKIES).json()
        _allbook = requests.get("https://litvinov-booking.herokuapp.com/api/v1/booking/", cookies=request.COOKIES).json()
        ...
        for s in sort:
            ...
            if datetime.datetime.strptime(s['date_end'], "%Y-%m-%d") > datetime.datetime.now() \
                ...
                ch = requests.get("https://litvinov-hotel.herokuapp.com/api/v1/hotels/{}"
                ...
                hh = requests.get("https://litvinov-hotel.herokuapp.com/api/v1/hotels/{}"
                                  .format(s['hotel_uid']), cookies=session.cookies).json()
                hist.append(s)
                histhotel.append(hh)
