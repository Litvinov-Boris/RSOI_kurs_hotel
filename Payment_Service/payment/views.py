...
def create(request):
    try:
        data = auth(request)
        loyBalance = requests.get("https://litvinov-loyalty.herokuapp.com/api/v1/loyalty/balance", cookies=request.COOKIES)
...
def pay(request, payment_uid):
    try:
        ...
        payment.status = "PAID"
        pay_loyalty = requests.patch("https://litvinov-loyalty.herokuapp.com/api/v1/loyalty/edit_balance",
